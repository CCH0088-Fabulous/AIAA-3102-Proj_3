...
def estimate_global_motion(prev_gray, curr_gray, mask=None, feature_params=None, lk_params=None):
        if mask is not None:
                norm_mask = normalize_mask(mask)
                if norm_mask is not None:
                        bg_mask = cv2.bitwise_not(norm_mask)
                else:
                        bg_mask = np.ones_like(prev_gray, dtype=np.uint8) * 255
        else:
                bg_mask = np.ones_like(prev_gray, dtype=np.uint8) * 255

        bg_feature_params = dict(feature_params) if feature_params else {}
        bg_feature_params["max_corners"] = bg_feature_params.get("max_corners", 200) * 2

        bg_features = extract_feature_points(prev_gray, bg_mask, bg_feature_params)
        if bg_features is None or len(bg_features) < 4:
                return np.eye(2, 3, dtype=np.float32)

        prev_pts, curr_pts, _, _ = track_points_lk(prev_gray, curr_gray, bg_features, lk_params)

        if len(prev_pts) < 4:
                return np.eye(2, 3, dtype=np.float32)

        transform, inliers = cv2.estimateAffinePartial2D(prev_pts, curr_pts, method=cv2.RANSAC)
...
def estimate_mask_motion(
        ...
        previous_points, current_points, raw_displacements, raw_magnitudes = track_points_lk(
                previous_gray,
                current_gray,
                feature_points,
                lk_params=lk_params,
        ...
        if raw_magnitudes.size == 0:
                empty_disp = np.empty((0, 2), dtype=np.float32)
                empty_mag = np.empty((0,), dtype=np.float32)
                return {
                        "valid": False,
                        ...
                        "median_motion": 0.0,
                        "max_motion": 0.0,
                        "displacements": empty_disp,
                        "magnitudes": empty_mag,
                        "points_previous": previous_points,
                        "points_current": current_points,
                }

        # Global motion compensation
        transform = estimate_global_motion(previous_gray, current_gray, mask, feature_params, lk_params)
        expected_points = cv2.transform(np.array([previous_points]), transform)[0]
        
        displacements = current_points - expected_points
        magnitudes = np.linalg.norm(displacements, axis=1).astype(np.float32)
